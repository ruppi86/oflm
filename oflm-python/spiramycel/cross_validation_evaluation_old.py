...
try:
    # Prefer relative imports when running as part of the spiramycel pkg
    from .neural_trainer import SpiramycelNeuralModel, NetworkConditions, load_spiramycel_parameters
    from .glyph_codec import SpiramycelGlyphCodec
    NEURAL_AVAILABLE = True
except ImportError:
    try:
        # Fallback to absolute imports (script executed from inside package dir)
        from neural_trainer import SpiramycelNeuralModel, NetworkConditions, load_spiramycel_parameters  # type: ignore
        from glyph_codec import SpiramycelGlyphCodec  # type: ignore
        NEURAL_AVAILABLE = True
    except ImportError:
        ...
        if Path(path).exists():
            try:
                if NEURAL_AVAILABLE:
                    ...
                    try:
                        # Use already-resolved helper from earlier import logic
...
def generate_glyphs_for_conditions(model, conditions, model_name, scenario_name):
    ...
    if NEURAL_AVAILABLE and hasattr(model, 'forward'):
        ...
        try:
            import torch
            try:
                from .neural_trainer import START_TOKEN, END_TOKEN, PAD_TOKEN  # type: ignore
            except ImportError:
                from neural_trainer import START_TOKEN, END_TOKEN, PAD_TOKEN  # type: ignore
...
def predict_effectiveness(model, conditions):
    ...
    if NEURAL_AVAILABLE and hasattr(model, 'forward'):
        ...
        try:
            import torch
            try:
                from .neural_trainer import START_TOKEN  # type: ignore
            except ImportError:
                from neural_trainer import START_TOKEN  # type: ignore
...
def perform_statistical_analysis(all_results):
    ...
    # Safely format p-value – could be None when variance is insufficient
    pval_str = f"{overall_p_value:.4f}" if isinstance(overall_p_value, (int, float)) else "N/A"
